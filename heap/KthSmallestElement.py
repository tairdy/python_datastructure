...
def find_kth_smallest(num, k):
    # first, put it in a max_heap
    max_heap = MaxHeap()
    for i in num:
        max_heap.insert(i)
    # print(f'heap list is {max_heap.heap}')
    for _ in range(len(max_heap.heap) - k + 1):
        popped_value = max_heap.remove()
    # print(f'kth smallest value = {popped_value}')
    return popped_value


# Test cases
nums = [[3, 2, 1, 5, 6, 4], [6, 5, 4, 3, 2, 1], [
    1, 2, 3, 4, 5, 6], [3, 2, 3, 1, 2, 4, 5, 5, 6]]
ks = [2, 3, 4, 7]
