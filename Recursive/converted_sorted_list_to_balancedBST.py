...
class BinarySearchTree:
    ...
    def __sorted_list_to_bst(self, nums, left, right):
        ...
        length_of_list = right - left + 1
        print(f"length_of_list = {length_of_list}")
        if length_of_list < 1:
            return None
        middle_node_index = int((right + left) / 2)
        if length_of_list == 3:
            middle_node = Node(nums[middle_node_index])
            middle_node.left = Node(nums[left])
            middle_node.right = Node(nums[right])
            return middle_node

        elif length_of_list == 2:
            middle_node = Node(nums[middle_node_index])
            middle_node.left = None
            middle_node.right = Node(nums[right])
            return middle_node
        elif length_of_list == 1:
            middle_node = Node(nums[left])
            middle_node.left = None
            middle_node.right = None
            return middle_node
        else:
            middle_node = Node(nums[middle_node_index])
            middle_node.left = self.__sorted_list_to_bst(
                nums, left, middle_node_index - 1
            )
            middle_node.right = self.__sorted_list_to_bst(
                nums, middle_node_index + 1, right
            )
            return middle_node
